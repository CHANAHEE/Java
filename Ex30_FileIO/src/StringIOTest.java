...
//				fw.close();
				...
				writer.println(s);
				writer.flush(); // @@ flush() 는 write() 할 때 버퍼가 생성되고, 생성된 버퍼에 남아있는 찌꺼기를 없애주는 메서드임. 
				writer.close();
...
//				fw.close(); // 9_ 써도 되고 안써도 보조스트림만 닫아도 자동으로 처리해준다.
		...
		try {
			...
			while(true) {
				String line = reader.readLine(); // 16_ 한줄씩 읽기 : 줄바꿈 문자는 제외됨.
				if(line == null) break;
				  
				// 17_ 대량의 데이터를 저장하는 datas List에 요소로 추가시키기 
				datas.add(line);
