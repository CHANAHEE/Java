...
class CThread extends Thread {
	...
	public void stopThread() {
		...
		synchronized (this) {
			notify(); // 23_ 반드시 퇴근을 시키려면 휴게실에 있는놈들을 다 깨워야 한다. 
	...
	public void resumeThread() {
		...
		synchronized (this) {
			notify(); // 22_ 동기화 필수! wait 명령과 겹칠수 있으니까!!
			
		}
		// this.notify(); // 21_this 없이 notify 하면 , 메인 스레드가 resumeThread 를 부르기 때문에 메인 스레드를 꺠우는것이 된다. 그래서 꼭 this 를 붙여줘야 한다.
	}
}
